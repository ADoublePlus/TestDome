/*
    Binrary search tree (BST) is a binary tree where the value of each node is larger or equal to the values in all the nodes
...
public class BinarySearchTree
{
    ...
    {
        if (root == null)
            return false; // Root is empty so cannot traverse

        if (root.Value == value)
            return true; // Value is on the root itself so need to traverse any tree
